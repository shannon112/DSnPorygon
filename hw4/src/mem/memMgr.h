...
//
// To promote 't' to the nearest multiple of SIZE_T (fast ceiling)
// e.g. Let SIZE_T = 8;  toSizeT(7) = 8, toSizeT(12) = 16
#define toSizeT(t)   ((t+SIZE_T_1)/SIZE_T)*SIZE_T  // TODO...ok
//
// To demote 't' to the nearest multiple of SIZE_T
// e.g. Let SIZE_T = 8;  downtoSizeT(9) = 8, downtoSizeT(100) = 96
#define downtoSizeT(t)  (t/SIZE_T)*SIZE_T  // TODO...ok
...
class MemBlock
{
   ...
   MemBlock(MemBlock<T>* n, size_t b) : _nextBlock(n) {
      ...
      //using char array(pointer) give a 1byte*b size memory
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO...ok
      size_t t_resized = toSizeT(t);
      if (getRemainSize() < t_resized) return false;
      else{
         ret = (T*)_ptr;
         _ptr += t_resized;
         return true;
...
class MemRecycleList
{
   ...
   // Member functions
   // ----------------
   size_t getArrSize() const { return _arrSize; } //which size-type trash can
   ...
   T* popFront() {
      // TODO...ok
      // take out pointer stored at _first first byte in size_t 
   ...
   void  pushFront(T* p) {
      // TODO...ok
      // p first byte is stored pointer in size_t form(decimal) point to previous p, now _first point to p 
...
public:
   MemMgr(size_t b = 65536) : _blockSize(b) {
      ...
      for (int i = 0; i < R_SIZE; ++i)
         _recycleList[i]._arrSize = i; //different size trash can
   }
   ...
   T* alloc(size_t t) {
      ...
      #endif // MEM_DEBUG
      // Note: no need to record the size of the array == > system will do (that size_t t is it, indicate how many bytes it needs)
      return getMem(t);
   }
   // Called by new[]
   T* allocArr(size_t t) {
   ...
   void  freeArr(T* p) {
      ...
      // and delete get the ptr start at size not size+8, 
      // so we dont need to p-1 get the size,
...
private:
   ...
   size_t getArraySize(size_t t) const {
      ...
      assert(t % SIZE_T == 0);
      assert(t >= S);
      if (t==toSizeT(S)) return 0; //single object size = 0
   ...
   // MemRecycleList like 256 mailboxes, each mailboxes not only content itself but also same series 
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      // TODO
      size_t m = n % R_SIZE;
      MemRecycleList<T>* recycleList_now = &_recycleList[m];
      while(1){
         if (recycleList_now->getArrSize()==n) return recycleList_now;
         else if(recycleList_now->getNextList()==0){
            MemRecycleList<T>* recycleList_new = new MemRecycleList<T>(n);
            recycleList_now->setNextList(recycleList_new);
            return recycleList_new;
         }
         else recycleList_now = recycleList_now->getNextList();
      }
   }
   // t is the #Bytes requested from new or new[]
   // Note: Make sure the returned memory is a multiple of SIZE_T
   T* getMem(size_t t) {
      T* ret = 0; //the return pointer
      #ifdef MEM_DEBUG
      ...
      // TODO
      else if (_activeBlock->getMem(t,ret)) { /*address sent to ret*/}
      else{ //false to getMem, not enough
         size_t remain = _activeBlock->getRemainSize();
         if (remain>=toSizeT(S)){ //if recycle-able
            T* temp;
            size_t rn = getArraySize(remain);
            size_t temp_size = rn ? rn*S+SIZE_T : S;
            _activeBlock->getMem(temp_size,temp);
            getMemRecycleList(rn)->pushFront(temp);
            #ifdef MEM_DEBUG
